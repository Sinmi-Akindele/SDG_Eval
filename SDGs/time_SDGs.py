...
from torch import nn, from_numpy, randint, normal, sqrt, device, save
...
class Generator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.rnn = nn.GRU(input_size=opt.embed_dim, hidden_size=opt.embed_dim, num_layers=opt.num_layer, batch_first=True)
...
# Function to fetch the model based on the backbone
def fetchModel(in_features, args):
    model = None
    if args.backbone.lower() == 'timegan':
        model = TimeGAN(args)
...
def TimeGAN_generate():
  # Hardcoding the arguments
  args = type('Args', (object,), {})()
  args.backbone = "timegan"
  args.batch_size = 1024
  args.embed_dim = 128  # Increased embedding dimension
  args.lr = 1e-4
  args.beta1 = 0.9
  args.num_layer = 3
  args.epochs = 1000
  args.window_size = 32
  args.stride = 1
  args.synth_mask = "mask"           # Used for organizing output paths
  args.n_trials = 5 

  # Map from dataset name to unwanted columns
  cols_to_drop = {
  	"MetroTraffic":   ['date_time', 'holiday', 'weather_main', 'weather_description'],
    "Energy":         ['date'],
    "Tesla":          ['Id', 'Date'],
  }

  for ds in ["MetroTraffic","Energy","Tesla"]:
    args.dataset = ds

    # Load the dataframe
    df = pd.read_csv(os.path.join(basePath, f"{args.dataset}.csv"))

    # Drop unnecessary columns
    df.drop(columns=cols_to_drop[ds], inplace=True)

    # Normalize the data before training (using MinMaxScaler)
    scaler = MinMaxScaler(feature_range=(-1, 1))  # Instead of (0, 1)
    df_scaled = pd.DataFrame(scaler.fit_transform(df), columns=df.columns)

    # Use 100% of the dataset for training
    training_df = df_scaled.copy()
    real_df = df.copy()

    # Padding if needed to ensure the data is evenly divisible by window size
    extra_samples = None
    if (len(training_df) % args.window_size) > 0:
        extra_samples = args.window_size - (len(training_df) % args.window_size)
        last_vals = training_df.iloc[-1:, :].values
        pad_vals = np.tile(last_vals, (extra_samples, 1))
        new_rows = pd.DataFrame(pad_vals, columns=training_df.columns)

        # Append the extra rows to the synthetic data
    ...
    m_vals = np.array([True] * len(training_df))

    # Mark the padding rows as False in the mask
    if extra_samples is not None:
        m_vals[-extra_samples:] = False
        # Ensure the mask tensor has the same shape as the data tensor
    ...
    training_dataset = MyDataset(from_numpy(np.array(windows)).float())
    ...
    model = fetchModel(in_dim, args).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataloader:
            ...
            loss = model.l_mse(recovered, batch)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataloader:
            ...
            total_loss += loss.detach().cpu().item()
        print(f'[Supervisor] Epoch {epoch}, Loss: {total_loss:.4f}')
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataloader:
            batch = batch.to(device)
            z = torch.normal(0, 1, (batch.size(0), batch.size(1), args.embed_dim)).to(device)
            ...
            err_g_V1 = torch.mean(torch.abs(torch.std(out_rsg, dim=[0, 1]) - torch.std(batch, dim=[0, 1])))
            err_g_V2 = torch.mean(torch.abs(torch.mean(out_rsg, dim=[0, 1]) - torch.mean(batch, dim=[0, 1])))
            err_s = model.l_mse(out_se[:, :-1, :], out_e[:, 1:, :])

            loss_g = err_g_U + err_g_U_e + err_g_V1 * 100 + err_g_V2 * 100 + torch.sqrt(err_s)
            ...
            loss_d = err_d_real + err_d_fake + err_d_fake_e
        ...
        print(f'[Generator] Epoch {epoch}, Loss_G: {total_loss_g:.4f}, Loss_ER: {total_loss_er:.4f}, Loss_D: {total_loss_d:.4f}')

        # Generation
        # Run synthetic data generation for n_trials
        for trial in range(args.n_trials):
            with torch.no_grad():
                # Initialize an empty tensor to collect the synthetic samples
                synth_tensor = torch.empty(0, training_dataset.inputs.shape[2]).to(device)

                # Iterate over batches in test data
                for idx, (test_batch, mask_batch) in enumerate(zip(dataloader, mask_dataloader)):
                	test_batch = test_batch.to(device)
                	mask_batch = mask_batch.to(device)

                	# Use real sequence as generator input
                	z = torch.randn_like(test_batch[:, :, :args.embed_dim])  # shape: [batch_size, seq_len, embed_dim]
                	out_g = model.netg(z, tanh=True)
                	out_r = model.netr(out_g) # Reconstruct real-valued outputs from latent
            ...
            synth_np = scaler.inverse_transform(synth_np)
            synth_np = np.clip(synth_np, real_df.min().values, real_df.max().values)

            # Save synthetic data
            df_synthesized = pd.DataFrame(synth_np, columns=df.columns)

            # Remove extra padded rows if they were added earlier
            if extra_samples is not None:
                df_synthesized = df_synthesized.iloc[:-extra_samples, :]
            
            # Reset index for clean CSV export
            df_synthesized = df_synthesized.reset_index(drop=True)
            real_df_reconverted = real_df.reset_index(drop=True)

            # Define where to save the output
            os.makedirs(savePath, exist_ok=True)

            # Save synthetic data from this trial
            df_synthesized[real_df_reconverted.columns].to_csv(
                f'{savePath}timegan_output_{ds}_trial_{trial}.csv', index=False
            )
...
def TSDiff_generate():
    ...
    for ds in ["MetroTraffic","Energy","Tesla"]:
        ...
        # Padding if needed to ensure the data is evenly divisible by window size
        extra_samples = None
        if (n % args.window_size) > 0:
        	extra_samples = args.window_size - (n % args.window_size)
        	last_vals = df_synth.iloc[-1:, :].values
        	pad_vals = np.tile(last_vals, (extra_samples, 1))
        	new_rows = pd.DataFrame(pad_vals, columns=df_synth.columns)

        	# Append the extra rows to the synthetic data
        	df_synth = pd.concat([df_synth, new_rows], ignore_index=True)

        # Convert NumPy arrays to PyTorch tensors
        x = torch.from_numpy(df_synth.values).float().to(device) 

        # Create windows of input data and masks
        windows = x.unfold(0, args.window_size, args.window_size)

        # Prepare training dataset and dataloader
        dataset = MyDataset(windows)
        dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True)

        # Build and move model to device
        model = fetchModel(windows.shape[-1], args).to(device)
        diffusion_config = fetchDiffusionConfig(args)
        optimizer = optim.Adam(model.parameters(), lr=args.lr)
        criterion = nn.MSELoss()

        # Move everything to GPU if available
        for key in ['betas', 'alphas', 'alpha_bars']:
            diffusion_config[key] = diffusion_config[key].to(device)
        ...
        for epoch in range(args.epochs):
            ...
            for batch in dataloader:
                ...
                timesteps = torch.randint(0, diffusion_config['T'], (B,), device=device)
                sigmas = torch.normal(0, 1, size=batch.shape).to(device)

                # Fetch and reshape alpha_bar for broadcasting
                alpha_bars = diffusion_config['alpha_bars'][timesteps].view(B, 1, 1).to(device)
                batch_noised = torch.sqrt(alpha_bars) * batch + torch.sqrt(1 - alpha_bars) * sigmas

                # Predict noise: model expects (L, B, F)
                sigmas_predicted = model(batch_noised.permute(1,0,2)).permute(1,0,2)
        ...
        for trial in range(args.trials):
            # Use high-entropy UUID-based seed
            ...
            # Loop through all batches in test and mask dataloaders
            for batch in dataloader:
                B, L, F = batch.shape

                # Initial noise sample per batch
                x = torch.randn((B, L, F), device=device)

                for t in reversed(range(diffusion_config['T'])):
                    a_t     = diffusion_config['alphas'][t]
                    a_bar_t = diffusion_config['alpha_bars'][t]
                    b_t     = diffusion_config['betas'][t]

                    # predict noise ε
                    with torch.no_grad():
                        eps = model(x.permute(1,0,2)).permute(1,0,2)

                    # one DDPM update
                    coef = b_t / torch.sqrt(1 - a_bar_t)
                    x = (x - coef * eps) / torch.sqrt(a_t)

                    # add variance if t>0
                    if t > 0:
                        var = b_t * (1 - a_bar_t) / (1 - diffusion_config['alpha_bars'][t-1])
                        x = x + torch.sqrt(var) * torch.randn_like(x)

                all_windows.append(x.cpu())

            elapsed = timer() - start
            print(f"  generation took {elapsed:.1f}s")

            # Stitch windows together and drop padding
            synth = torch.cat(all_windows, dim=0)               # (num_windows, window_size, in_dim)
            synth = synth.reshape(-1, in_dim)[:n]               # flatten & trim to original n rows

            # Inverse-transform and write CSV
            synth_np = synth.numpy()
            denorm   = np.clip(scaler.inverse_transform(synth_np), df.min().values, df.max().values)
            scaler.inverse_transform(synth_np)
            df_out   = pd.DataFrame(denorm, columns=df.columns)

            # Reset index for clean CSV export
            df_out = df_out.reset_index(drop=True)

            # Save synthetic data from this trial
            out_path = os.path.join(savePath, f"tsdiff_output_{ds}_trial_{trial}.csv")
            df_out.to_csv(out_path, index=False)
            print(f"  Generated {n} rows in {elapsed:.1f}s")
