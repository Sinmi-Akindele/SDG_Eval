...
synthetic_cases = [
    {"synthetic_model": "PAR", "dataset": "PIAAC_wide"},
    {"synthetic_model": "CTGAN", "dataset": "PIAAC_wide"},
    {"synthetic_model": "CTGAN", "dataset": "PIAAC_numeric_decoding"},
    {"synthetic_model": "CTGAN", "dataset": "PIAAC_auto_decoding"}
]

seeds = [42, 50, 61, 79, 83]
datasets = {"PIAAC_wide", "PIAAC_numeric_decoding", "PIAAC_auto_decoding"}
...
for file_name in os.listdir(basePath):
    ...
    if metric == "accuracy":
        score = accuracy_score(df['outputs_true'], df['outputs_pred'])
    else:
...
results_df.to_csv(os.path.join(filePath, "compiled_eval_real_synthetic_type2a_PIAAC_results.csv"), index=False)
