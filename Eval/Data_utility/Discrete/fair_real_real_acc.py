...
for file_name in os.listdir(basePath):
    if file_name.endswith(".csv"):
        ...
        if metric == "accuracy":
            minority_score = accuracy_score(minority_df['y_true'], minority_df['y_pred'])
            majority_score = accuracy_score(majority_df['y_true'], majority_df['y_pred'])
        else:
            minority_score = None
            majority_score = None
...
results_df.to_csv(os.path.join(filePath, "compiled_fairness_real_real_type1_results.csv"), index=False)
